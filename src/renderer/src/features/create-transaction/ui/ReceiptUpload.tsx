...
/** Fields we can lift from a parsed receipt into the transaction form. */
...
const ReceiptUpload = ({
  ...
  const handleScan = async (): Promise<void> => {
    ...
    try {
      ...
      const { text } = await window.visionAPI.parse(
        ...
        {previewUrl ? (
          ...
          <span className="px-6 text-center">
            ...
            <span className="text-xs text-zinc-600">
              It’s read on-device, never uploaded
            </span>
          </span>
        )}
      </button>
...
const mapReceipt = (text: string): ReceiptPrefill => {
  ...
  else if (typeof json.total === "string" && json.total.trim())
    prefill.amount = json.total.trim();
